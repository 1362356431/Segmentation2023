...
if __name__ == "__main__":
    ...
    net = qtnUnet(img_size=256, in_chans=3, embed_dims=[16, 64, 128, 256], mlp_ratios=[8, 8, 4, 4],
                  norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 5, 2]).cuda()
    # net = VisionTransformer(num_classes=2, config=config1).cuda()
    # net = ViT_seg(config=config,img_size=256,num_classes=2).cuda()
    ...
    snapshot = os.path.join(args.output_dir, 'epoch_120.pth')
