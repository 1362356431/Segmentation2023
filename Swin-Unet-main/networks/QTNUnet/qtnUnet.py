...
class SegmentationHead(nn.Module):
    def __init__(self,num_classes=2,inchannels=16):
        ...
        self.cov = nn.Conv2d(in_channels=16,out_channels=num_classes,kernel_size=1,padding=0,stride=1)
    def forward(self,x):
        x = self.cov(x)
        x = self.upCov(x)
        return x
...
class qtnUnet(nn.Module):
    def __init__(self,img_size=256, in_chans=3, embed_dims=[16, 64, 128, 256], mlp_ratios=[8, 8, 4, 4],
        ...
        self.encoder = Encoder(img_size=256, in_chans=3, embed_dims=[16, 64, 128, 256], mlp_ratios=[8, 8, 4, 4],
                norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 5, 2],)
        self.decoder = Decoder(img_size=16, embed_dims=[128, 64, 16], mlp_ratios=[8, 8, 4, 4],
                norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 5, 2])
